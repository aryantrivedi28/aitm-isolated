...
const defaultIcons = [AlertTriangle, Clock, Layers]

interface PainPointsProps {
  heading?: string
  points?: { title: string; body: string; icon?: keyof typeof defaultIcons | string }[]
}

export default function PainPoints({ heading, points = [] }: PainPointsProps) {
  return (
    <section className="bg-[#241C15] py-20 text-white">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="mt-10 grid gap-8 grid-cols-1 md:grid-cols-3">
          {points.map((p, i) => {
            const Icon = defaultIcons[i % defaultIcons.length] || AlertTriangle
            return (
              <motion.div
                key={i}
              ...
              >
                <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto -mt-12 bg-gradient-to-br from-[#FFE01B] to-[#FFD700]/80 shadow-lg">
                ...
                <p className="mt-3 text-gray-300 text-center">{p.body}</p>
