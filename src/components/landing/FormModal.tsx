...
import { X, ArrowRight, Loader2, AlertTriangle, CheckCircle2, Sparkles, Star, Zap, Rocket, Shield, TrendingUp } from 'lucide-react'
...
export default function FormModal({ formConfig, buttonText = "Get Started", buttonClassName = "" }: FormModalProps) {
    ...
    try {
      const response = await fetch('/api/submit', {
        ...
        body: JSON.stringify({
          ...
          pageSlug: window.location.pathname.replace('/', ''),
          data: formData,
        }),
      })
  ...
  const validateForm = (data: Record<string, any>): boolean => {
    ...
    if (formConfig.additionalFields) {
      for (const field of formConfig.additionalFields) {
        if (field.required && !data[field.name]) return false
      }
    }
      ...
      <div className="max-w-3xl mx-auto text-center">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-medium text-[#050504] mb-4">
            Thank You, <span className="text-[#f7af00]">Valued Client</span>!
          </h2>
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 md:p-8 shadow-lg border border-[#f7af00]/20">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center space-x-3 text-[#31302f]">
                ...
                <span>Preparing your personalized consultation</span>
              </div>
            </div>
          </div>
        </motion.div>
  ...
  return (
    <>
      ...
      <button
        ...
        className={`inline-flex items-center justify-center gap-3 px-8 py-4 rounded-2xl font-semibold transition-all duration-300 bg-[#f7af00] text-[#241C15] shadow-2xl hover:shadow-3xl hover:scale-[1.02] ${buttonClassName}`}
      >
        <span>{buttonText}</span>
        <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
      </button>

      {/* Modal - Same across all pages */}
      <AnimatePresence>
        {showForm && (
          <>
            <motion.div
                    ...
                    <div className="p-6 md:p-8 lg:p-10">
                      <div className="mb-8">
                        <h3 className="text-2xl md:text-3xl font-medium text-[#050504] mb-3">
                          {formConfig?.title || 'Get Your Free Consultation'}
                        </h3>
                        <p className="text-[#31302f]">
                          {formConfig?.description || 'Fill out this form and our experts will contact you within 24 hours.'}
                        </p>
                      </div>
